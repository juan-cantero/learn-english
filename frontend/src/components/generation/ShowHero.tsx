  ...
  return (
    <div className="flex flex-col gap-6 sm:flex-row sm:gap-8">
      ...
      <div className="mx-auto w-48 flex-shrink-0 sm:mx-0 sm:w-56">
        <div className="aspect-[2/3] overflow-hidden rounded-xl bg-bg-dark shadow-xl">
          {show.posterUrl ? (
            <img
              src={show.posterUrl}
              alt={show.title}
      ...
      <div className="flex-1 text-center sm:text-left">
        ...
        <div className="mb-4 flex flex-wrap justify-center gap-2 sm:justify-start">
          <span className="inline-flex items-center rounded-full bg-accent-primary/10 px-3 py-1 text-sm font-medium text-accent-primary">
            {seasons.length} {seasons.length === 1 ? 'Season' : 'Seasons'}
          </span>
          <span className="inline-flex items-center rounded-full bg-bg-card px-3 py-1 text-sm font-medium text-text-secondary">
            {totalEpisodes} Episodes
          </span>
        </div>

        {show.overview && (
          <p className="text-text-secondary line-clamp-4 sm:line-clamp-none">
            {show.overview}
          </p>
        )}
      </div>
